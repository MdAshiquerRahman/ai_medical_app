...
import 'package:ai_medical_app/features/scan_analysis/data/preprocessing/chest_ct_scan_preprocessor.dart';

class ChestCTScanModelService implements MLModelService {
  final ChestCTScanPreprocessor _preprocessor;
  Interpreter? _interpreter;

  ChestCTScanModelService({ChestCTScanPreprocessor? preprocessor})
      : _preprocessor = preprocessor ?? ChestCTScanPreprocessor();

  @override
  ScanType get scanType => ScanType.chestCTScan;

  @override
  ModelConfig get config => const ModelConfig(
        modelPath: MLModelConstants.chestCTScanModelPath,
        classLabels: MLModelConstants.chestCTScanClasses,
        ...
        batchSize: MLModelConstants.chestCTScanBatchSize,
      );
  ...
  Future<Result<void>> loadModel() async {
    try {
      ...
      throw ModelLoadException('Failed to load Chest CT Scan model: $e');
    }
  }
